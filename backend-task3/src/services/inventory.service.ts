...
export class InventoryService {
  ...
  static async getInventory(ids: number[], delayMs?: number, fixed?: boolean) {
    if (!ids.length) return { count: 0, results: [] as Array<{ id: number; stock: number | string }>};

    // 1) Truy vấn theo TẬP
        // (Nếu InventoryModel đã có hàm findByProductIds(ids) thì dùng trực tiếp;
        // nếu không, bạn có thể chunk tại đây.)
    const rows: Array<{ productId: number; stock: number | null }> =
          await InventoryModel.findByProductIds(ids);
    const invMap = new Map<number, number | null>(
          rows.map(r => [r.productId, r.stock])
        );
